      ...
      <div className="mt-3 sm:mt-4">
        ...
        >
          {popularTags.map((tag, index) => (
            <Link
              key={index}
              href={`/tour-search-4?tag=${tag}`}
              className="flex-shrink-0 inline-flex items-center justify-center px-2.5 py-1 sm:px-3 sm:py-1.5 bg-gray-50 border-2 border-gray-200 rounded-xl text-xs sm:text-sm font-medium text-gray-500 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-all duration-200 whitespace-nowrap"
            >
              {tag}
            </Link>
          ))}
