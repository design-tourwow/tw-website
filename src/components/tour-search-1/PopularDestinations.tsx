...
export default function PopularDestinations() {
  ...
  return (
    <section className="mt-8 mb-6">
      ...
      {/* Popular Tags */}
      <div className="mt-4">
        <div className="flex gap-2 overflow-x-auto pb-2 pr-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent hover:scrollbar-thumb-gray-400">
          {popularTags.map((tag, index) => (
            <Link
              key={index}
              href={`/tour-search-1?tag=${tag}`}
              className="flex-shrink-0 inline-flex items-center justify-center px-3 py-1.5 bg-white border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-all duration-200 whitespace-nowrap shadow-sm hover:shadow-md"
            >
              {tag}
            </Link>
          ))}
