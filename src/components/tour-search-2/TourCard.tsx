...
export default function TourCard({ tour }: TourCardProps) {
  return (
    <Link 
      ...
      className="block group cursor-pointer w-full sm:w-[calc(50%-0.625rem)] lg:flex-grow lg:min-w-[392px] lg:max-w-[450px]"
    >
      <div className="relative bg-gray-50 rounded-2xl px-3 py-3 sm:px-4 sm:py-4">
        <div className="relative aspect-[5/6] overflow-hidden rounded-xl shadow-md hover:shadow-xl transition-all duration-300">
          ...
          <div className="absolute top-2 sm:top-3 left-2 sm:left-3 z-20">
            <div className="bg-gradient-to-r from-white/95 to-white/90 backdrop-blur-md rounded-lg sm:rounded-xl shadow-xl overflow-hidden">
              <div className="flex items-stretch">
                <div className={`flex items-center gap-1.5 sm:gap-2 px-1.5 sm:px-2 py-1.5 sm:py-2 border-r ${tour.badgeColor === 'red' ? 'border-red-100' : 'border-blue-100'}`}>
                  <div className="relative">
                    ...
                    <div className={`relative bg-gradient-to-br ${tour.badgeColor === 'red' ? 'from-red-500 to-rose-600' : 'from-blue-500 to-blue-600'} p-1.5 sm:p-2 rounded-full w-6 h-6 sm:w-7 sm:h-7 flex items-center justify-center`}>
                ...
                <div className={`px-1.5 sm:px-2 py-1.5 sm:py-2 bg-gradient-to-r ${tour.badgeColor === 'red' ? 'from-red-50 to-rose-50' : 'from-blue-50 to-blue-50'} flex flex-col justify-center`}>
                  <p className="text-xs sm:text-sm text-blue-600 font-medium mb-0">ช่วงเดินทาง</p>
                  <p className={`text-sm sm:text-base font-bold ${tour.badgeColor === 'red' ? 'text-red-900' : 'text-blue-900'} -mt-1 whitespace-nowrap`}>{tour.travelPeriod}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="absolute top-0 right-0 z-20">
            <div className={`bg-gradient-to-bl ${tour.badgeColor === 'red' ? 'from-red-600 to-red-700' : 'from-blue-600 to-blue-700'} text-white px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-bl-lg sm:rounded-bl-xl shadow-md`}>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-3 sm:p-4 text-white z-10">
            <div className="mb-1.5 sm:mb-2">
              <div className={`${tour.badgeColor === 'red' ? 'bg-red-600' : 'bg-blue-600'} text-white px-2.5 sm:px-3 py-0.5 sm:py-1 rounded text-xs sm:text-sm font-bold inline-block shadow-lg`}>
                {tour.badgeText}
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 mb-1 sm:mb-1.5 text-sm sm:text-base">
              ...
              <span className="text-sm sm:text-base font-medium">{tour.rating} ({tour.reviews} รีวิว)</span>
            </div>
