    ...
    <section className="mt-6 sm:mt-8 mb-4 sm:mb-6">
      <div className="mb-3 sm:mb-4">
        ...
        <p className="text-xs text-gray-600">
          สำรวจทัวร์ญี่ปุ่นยอดนิยมแยกตามเมืองและช่วงเวลา
        </p>
      </div>
      ...
      <div className="mt-3 sm:mt-4">
        <div className="flex gap-1.5 sm:gap-2 overflow-x-auto pb-2 pr-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent hover:scrollbar-thumb-gray-400">
          {popularTags.map((tag, index) => (
            <Link
              key={index}
              href={`/tour-search-1?tag=${tag}`}
              className="flex-shrink-0 inline-flex items-center justify-center px-2.5 py-1 sm:px-3 sm:py-1.5 bg-white border border-gray-200 rounded-xl text-xs sm:text-sm font-medium text-gray-700 hover:bg-blue-50 hover:border-blue-300 hover:text-blue-700 transition-all duration-200 whitespace-nowrap shadow-sm hover:shadow-md"
            >
              {tag}
            </Link>
          ))}
